...
var Sonos = function Sonos(host, port) {
  ...
  fs.readdirSync(__dirname + '/services').forEach(function(filename) {
    var filenameMatch = filename.match(/^(.+)\.js$/)[1];

    var lowerCaseFilenameMatch = filenameMatch.substring(0,1).toLowerCase() + filenameMatch.substring(1, filenameMatch.length);

    that[lowerCaseFilenameMatch] = new Services[filenameMatch](that.host, that.port);
  });
